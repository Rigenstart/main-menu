...
echo -e "${GREEN}██      ██      ██ ███████     ███████ ██  ██████ ███████ ██   ████ ███████ ███████ ${STD}"
				  ...
				  echo -e "${BLUE} WHASTAPPS 6282271426251${STD}"				    
                                                                                                   
	echo "${YELLOW}[${WHITE}0${YELLOW}] ${BLUE}Keluar dari aplikasi"
	echo "${YELLOW}[${WHITE}1${YELLOW}] ${BLUE} Informasi dan Ringkasan Server"
	echo "${YELLOW}[${WHITE}2${YELLOW}] ${BLUE} Ping ke IP server || Validasi RBL"
	echo "${YELLOW}[${WHITE}3${YELLOW}] ${BLUE} Buat ruang disk server"
	echo "${YELLOW}[${WHITE}4${YELLOW}] ${BLUE} Hentikan pelaku spam || hapus ANTRIAN email || Aktifkan SpamAssassin"
	echo "${YELLOW}[${WHITE}5${YELLOW}] ${BLUE} Instalar let's encryt"
	echo "${YELLOW}[${WHITE}6${YELLOW}] ${BLUE} Tetapkan TANGGAL/WAKTU || Aktifkan NTPD "
	echo "${YELLOW}[${WHITE}7${YELLOW}] ${BLUE} NGINX sebagai proksi terbalik (DirectAdmin)"
	echo "${YELLOW}[${WHITE}8${YELLOW}] ${BLUE} Bersihkan Seluruh ANTRIAN Email"
	echo "${YELLOW}[${WHITE}9${YELLOW}] ${BLUE} Instalar CSF Firewall"
	echo "${YELLOW}[${WHITE}10${YELLOW}] ${BLUE} Perbaiki semua Database || Optimalkan my.cnf || Parsing my.cnf dari SQL"
	echo "${YELLOW}[${WHITE}11${YELLOW}] ${BLUE} IMAPSync untuk 10 akun"
	echo "${YELLOW}[${WHITE}12${YELLOW}] ${BLUE} Izinkan tautan sementara menggunakan /~ (DirectAdmin)"
	echo "${YELLOW}[${WHITE}13${YELLOW}] ${BLUE} Perpanjang Lisensi DirectAdmin"
	echo "${YELLOW}[${WHITE}14${YELLOW}] ${BLUE} WGET dengan otentikasi FTP"
	echo "${YELLOW}[${WHITE}15${YELLOW}] ${BLUE} Instal MALDET dan CLAMAV || Jalankan Antivirus"
	echo "${YELLOW}[${WHITE}16${YELLOW}] ${BLUE} Hitung Ulang Kuota Pengguna di Directadmin"
	echo "${YELLOW}[${WHITE}17${YELLOW}] ${BLUE} PERBAIKI - Kesalahan Berkeley DB: /var/spool/exim/db/callout di EXIM"
	echo "${YELLOW}[${WHITE}18${YELLOW}] ${BLUE} Instal DIRECTADMIN | WHM/CPANEL (Server Bersih)"
	echo "${YELLOW}[${WHITE}19${YELLOW}] ${BLUE} Menguji email dengan SWAKS."
	echo "${YELLOW}[${WHITE}20${YELLOW}] ${BLUE} STABILISASI Server Penuh."
	echo "${YELLOW}[${WHITE}21${YELLOW}] ${BLUE} OPTIMALKAN Server Lengkap."
	echo "${YELLOW}[${WHITE}22${YELLOW}] ${BLUE} PERBARUI Server Penuh."
	printf "\n"
	echo -e "${GREEN}----------------------------------------------------------------------------------------------${STD}"
	printf "\n"
}
